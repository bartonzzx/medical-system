<template>
  ...
  <el-container>
    ...
    <el-main>
      ...
      <!-- 表格 -->
      <el-table :data="tableData.list" stripe :default-sort="{ prop: 'date', order: 'descending' }"
        ...
        <el-table-column label="操作" width="210px" v-if="hasRole">
          <template slot-scope="scope">
            <button class="table-btn-delete" @click="handleDeleteDoctor(scope.row.id, scope.row.name)" />
            <button class="table-btn-update" size="small" @click="
              ...
              " />
            <el-button plain size="mini" style="width: 75px" class="table-btn"
      ...
      <!-- 点击新增后的弹窗 -->
      <el-dialog title="新增医生信息" :visible.sync="addFormVisible" :modal-append-to-body="false" @close="handleAddClose">
        <el-form :model="addForm" hide-required-asterisk ref="addForm" label-width="110px">
          <el-form-item label="医生姓名" prop="name" :rules="doctorRules.nameRules">
            <el-input v-model.trim="addForm.name" autocomplete="off" required />
          </el-form-item>
          <el-form-item label="年龄" prop="age" :rules="doctorRules.ageRules">
            <el-input v-model.number="addForm.age" autocomplete="off" required />
          </el-form-item>
          ...
          <el-form-item label="级别" prop="levelId" :rules="doctorRules.requiredRules">
            <el-select v-model="addForm.levelId" placeholder="请选择级别">
              <el-option v-for="item in levelAndTypeData.allLevel" :key="item.id" :label="item.name" :value="item.id" />
            </el-select>
          </el-form-item>
          <el-form-item label="手机号" prop="phoneNumber" :rules="doctorRules.phoneRules">
            <el-input v-model="addForm.phoneNumber" autocomplete="off" required />
          </el-form-item>
          <el-form-item label="密码" prop="pwd" :rules="pwdRules">
            <el-input type="password" v-model.trim="addForm.pwd" autocomplete="off" required />
          </el-form-item>
          <el-form-item label="确认密码" prop="pwdCheck" :rules="checkPwdRules">
            <el-input type="password" v-model.trim="addForm.pwdCheck" autocomplete="off" required />
          </el-form-item>
          <el-form-item label="诊治类别" prop="typeId" :rules="doctorRules.requiredRules">
            <el-select v-model="addForm.typeId" placeholder="请选择诊治类别">
              <el-option v-for="item in levelAndTypeData.allTreatType" :key="item.id" :label="item.name"
                :value="item.id" />
            </el-select>
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="handleAddDoctor('addForm')">确定</el-button>
        </div>
      </el-dialog>
      <!-- 点击修改后的弹窗 -->
      <el-dialog title="修改医生信息" :visible.sync="modifyFormVisible" :modal-append-to-body="false"
        @close="handleModifyClose">
        <el-form :model="modifyForm" hide-required-asterisk ref="modifyForm" label-width="110px">
          <el-form-item label="医生姓名" prop="name" :rules="doctorRules.nameRules">
            <el-input v-model.trim="modifyForm.name" autocomplete="off" />
          </el-form-item>
          <el-form-item label="年龄" prop="age" :rules="doctorRules.ageRules">
            <el-input v-model.number="modifyForm.age" autocomplete="off" />
          </el-form-item>
          ...
          <el-form-item label="手机号" prop="phoneNumber" :rules="doctorRules.phoneRules">
            <el-input v-model="modifyForm.phoneNumber" autocomplete="off" required />
          </el-form-item>
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="handleModifyDoctor('modifyForm')">确定</el-button>
        </div>
      </el-dialog>
    </el-main>
  </el-container>
...
import { doctorRules, validatePass } from "../../utils/validator.js";
import { resetPassword } from "../../api/admin/doctorInfoManage.js";
