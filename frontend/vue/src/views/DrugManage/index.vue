  ...
  >
    ...
    >
      <el-upload
        ...
        :on-success="handiellploadSuccess"
        :on-error="handiellploadError"
        :on-progress="handiellploading"
      ...
      >
        ...
        <el-progress
          ...
          :status="status"
        ></el-progress>
        <i v-else class="el-icon-plus_drugimg-uploader-icon"></i>
      </el-upload>
    </el-form-item>
    <el-form-item
      label="销售药店"
...
>
    ...
    <el-form-item
        label="销售药店"
        prop="salelds"
        irules="rules.requiredRules"
    >
        <el-select
            v-model="modifyForm.salelds"
            multiple
            placeholder="请选择销售地点"
        >
            <el-option
...
export default {
  ...
  data() {
    return {
      ...
      status: null, // 进度条状态
      keyword: "", // 搜索关键字，建议补充
      salePlaceInfo: { list: [] }, // 销售地点，建议补充
      tableData: { list: [] }, // 表格数据，建议补充
    };
  },
  methods: {
    // 显示进度条
    ...
    handleCurrentChange(event) {
      ...
      if (this.keyword && this.keyword.length) {
        this.handle1Query(this.keyword);
      } else {
        this.getDrugInfo();
      }
    },
    // 关键字查询
    handle1Query(keyword) {
      this.$store.dispatch("drugInfoManage/getDrugInfo", {
        pn: this.currentPage,
        size: this.pageSize,
        keyword,
    ...
    handleModifyFormVisible(
      ...
      let saleIds = drugSales.map(item => item.saleId);
      this.modifyForm = {
        drugId,
        drugName,
        drugInfo,
