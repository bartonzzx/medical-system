    ...
    <el-container>
      ...
      >
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button
            type="primary"
            @click="handleAddMedicalPolicy('addForm')"
          >确定</el-button>
        </div>
      </el-dialog>
      <!-- 点击修改后的弹窗 -->
...
import rules, { searchRules } from "../../utils/validator.js";
  ...
  methods: {
    ...
    handleAddMedicalPolicy(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$store.dispatch(
            "medicalPolicyInfoManage/addMedicalPolicy",
            {
    ...
    handleDeletMedicalPolicy(id, title) {
      this.$confirm(`确定要删除 "${title}" 的相关信息吗？`, "提示", {
       ...
       .then(() => {
          this.$store.dispatch(
            "medicalPolicyInfoManage/deleteMedicalPolicy",
            {
              id,
              params: this.params,
            }
