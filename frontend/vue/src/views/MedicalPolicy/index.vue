...
<template>
  <el-container>
    ...
    <el-main>
          ...
          <el-form :model="searchLimit" ref="searchLimit">
            <div class="input-item">
              ...
              <el-form-item prop="id" :rules="searchRules.intRules">
                <el-input placeholder="请输入" size="small" v-model.number="searchLimit.id" maxlength="6" />
              </el-form-item>
            </div>
            <div class="input-item">
              <h5>政策标题</h5>
              <el-form-item prop="title">
                <el-input placeholder="请输入" size="small" v-model="searchLimit.title" maxlength="14" />
              </el-form-item>
            </div>
            <div class="input-item">
              <h5>发布时间</h5>
              <el-form-item prop="publishTime">
                <el-date-picker type="date" placeholder="选择日期" size="small" v-model="searchLimit.publishTime"
            ...
            <div class="input-item">
              ...
              <el-form-item prop="city">
                <el-select placeholder="请选择需要查询的城市" v-model="searchLimit.city" clearable>
                  <el-option v-for="city in cityInfo.list" :key="city.cityId" :label="city.city" :value="city.cityId" />
                </el-select>
              </el-form-item>
            </div>
          </el-form>
          <el-button type="primary" size="small" @click="handleLimitedSearch('searchLimit')">查找</el-button>
          <el-button type="primary" size="small" @click="addFormVisible = true" v-if="hasRole">新增</el-button>
        </div>
      </div>

      <!-- 表格区域 -->
      <div class="table-policy">
        <el-table :data="tableData.list" stripe :default-sort="{ prop: 'date', order: 'descending' }" max-height="260"
          ...
          <el-table-column prop="medical_policy_operation" label="操作" v-if="hasRole">
            <template slot-scope="scope">
              <button class="table-btn-delete" @click="handleDeleteMedicalPolicy(scope.row.id, scope.row.title)" />
      ...
      <!-- 新增弹窗 -->
      <el-dialog title="新增医保政策" :visible.sync="addFormVisible" :modal-append-to-body="false" @close="handleAddClose">
        <el-form :model="addForm" hide-required-asterisk ref="addForm" label-width="110px">
          ...
          <el-form-item label="政策内容" prop="message" :rules="rules.infoRules">
            <el-input v-model.trim="addForm.message" autocomplete="off" required autosize type="textarea" />
          </el-form-item>
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="handleAddMedicalPolicy('addForm')">确定</el-button>
        </div>
      </el-dialog>

      <!-- 修改弹窗 -->
      <el-dialog title="修改医保政策信息" :visible.sync="modifyFormVisible" :modal-append-to-body="false"
        ...
        <el-form :model="modifyForm" hide-required-asterisk ref="modifyForm" label-width="110px">
          ...
          <el-form-item label="政策内容" prop="message" :rules="rules.infoRules">
            <el-input v-model.trim="modifyForm.message" autocomplete="off" required autosize type="textarea" />
          </el-form-item>
          <el-form-item label="生效城市" prop="city" :rules="rules.requiredRules">
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="handleModifyMedicalPolicy('modifyForm')">确定</el-button>
        </div>
      </el-dialog>
    </el-main>
  </el-container>
